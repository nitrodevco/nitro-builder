...
export class RoomEngine implements IRoomEngine
{
    ...
    constructor()
    {
        this._sessionDataManager = new SessionDataManager();
        this._roomSessionManager = new RoomSessionManager(this);
        this._roomContentLoader = new RoomContentLoader(this._sessionDataManager);
    ...
    public createRoomInstance(roomId: number, roomMap: RoomMapData): void
    {
        ...
        if(!roomMap)
        {
            console.warn('Room property messages');
    ...
    private setupRoomInstance(roomId: number, roomMap: RoomMapData, floorType: string, wallType: string, landscapeType: string, worldType: string): IRoomInstance
    {
        ...
        if(window.NitroBuilderConfig['enable.avatar.arrow']) instance.createRoomObjectAndInitalize(RoomEngine.ARROW_OBJECT_ID, RoomEngine.ARROW_OBJECT_TYPE, RoomObjectCategory.CURSOR);
    ...
    public getRoomInstanceDisplay(roomId: number, id: number, width: number, height: number, scale: number): Container
    {
        ...
        if(canvas.geometry)
        {
            ...
            if(displayObject)
            {
                const overlay = new Container(Texture.EMPTY);
    ...
    public addFurnitureWallByTypeName(roomId: number, id: number, typeName: string, location: IVector3D, direction: IVector3D, state: number, extra: string, expires: number = -1, usagePolicy: number = 0, ownerId: number = 0, ownerName: string = '', realRoomObject: boolean = true): boolean
    {
        const instanceData = this.getRoomInstanceData(roomId);
        ...
        const furnitureData = new RoomFurnitureData(id, 0, typeName, location, direction, state, objectData, NaN, expires, usagePolicy, ownerId, ownerName, true, realRoomObject);
    ...
    public dispatchMouseEvent(canvasId: number, x: number, y: number, type: string, altKey: boolean, ctrlKey: boolean, shiftKey: boolean, buttonDown: boolean): void
    {
        ...
        const sprite = this.getOverlayIconSprite(this.getRenderingCanvasOverlay(canvas), RoomEngine.OBJECT_ICON_SPRITE);
            ...
            if(this._roomContentLoader)
            {
                ...
                if(category === RoomObjectCategory.UNIT)
                {
                    ...
                    if(type === 'pet')
                    {
                        ...
                        imageResult = this.getRoomObjectPetImage(petFigureData.typeId, petFigureData.paletteId, petFigureData.color, new Vector3d(180), 64, true, 0, petFigureData.customParts, posture);
                    }
                    else
                    {
                        imageResult = this.getGenericRoomObjectImage(type, instanceData, new Vector3d(180), 64, 0, null, stuffData, state, frameNumber, posture);
                    }
                }
                else
                {
                    imageResult = this.getGenericRoomObjectImage(type, colorIndex.toString(), new Vector3d(), 1, 0, instanceData, stuffData, state, frameNumber, posture);
                }
            }
        }
    ...
    public getRoomObjectImage(roomId: number, objectId: number, category: number, direction: IVector3D, scale: number, bgColor: number = 0): IImageResult
    {
        ...
        return this.getGenericRoomObjectImage(type, color, direction, scale, bgColor, extras, data, -1, -1, null, id);
    }

    public getFurnitureFloorIcon(typeId: number, extras: string = null, objectData: IObjectData = null): IImageResult
    {
        return this.getFurnitureFloorImage(typeId, new Vector3d(), 1, 0, extras, -1, -1, objectData);
    }

    public getFurnitureWallIcon(typeId: number, extras: string = null): IImageResult
    {
        return this.getFurnitureWallImage(typeId, new Vector3d(), 1, 0, extras);
    }

    public getFurnitureFloorImage(typeId: number, direction: IVector3D, scale: number, bgColor: number = 0, extras: string = null, state: number = -1, frameCount: number = -1, objectData: IObjectData = null): IImageResult
    {
        let type: string = null;
        let color = '';
        ...
        if(scale === 1)
        {
            return this.getGenericRoomObjectThumbnail(type, color, extras, objectData);
        }

        return this.getGenericRoomObjectImage(type, color, direction, scale, bgColor, extras, objectData, state, frameCount);
    }

    public getFurnitureWallImage(typeId: number, direction: IVector3D, scale: number, bgColor: number = 0, extras: string = null, state: number = -1, frameCount: number = -1): IImageResult
    {
        let type: string = null;
        let color = '';
        ...
        if(scale === 1)
        {
            return this.getGenericRoomObjectThumbnail(type, color, extras, null);
        }

        return this.getGenericRoomObjectImage(type, color, direction, scale, bgColor, extras, null, state, frameCount);
    }

    public getRoomObjectPetImage(typeId: number, paletteId: number, color: number, direction: IVector3D, scale: number, headOnly: boolean = false, bgColor: number = 0, customParts: IPetCustomPart[] = null, posture: string = null): IImageResult
    {
        let type: string = null;
        ...
        return this.getGenericRoomObjectImage(type, value, direction, scale, bgColor, null, null, -1, -1, posture);
    }

    public getGenericRoomObjectImage(type: string, value: string, direction: IVector3D, scale: number, bgColor: number = 0, extras: string = null, objectData: IObjectData = null, state: number = -1, frameCount: number = -1, posture: string = null, originalId: number = -1): IImageResult
    {
        if(!this._roomManager) return null;
    ...
    public getGenericRoomObjectThumbnail(type: string, param: string, extraData: string = null, stuffData: IObjectData = null): IImageResult
    {
        if(!this._roomManager) return null;
    ...
    public setObjectMoverIconSpriteVisible(flag: boolean): void
    {
        const canvas = this.getActiveRoomInstanceRenderingCanvas();
    ...
    public removeObjectMoverIconSprite(): void
    {
        ...
        this.removeOverlayIconSprite(this.getRenderingCanvasOverlay(canvas), RoomEngine.OBJECT_ICON_SPRITE);
    }

    private getRenderingCanvasOverlay(renderingCanvas: IRoomRenderingCanvas): Container
    {
        if(!renderingCanvas) return null;

        return (renderingCanvas.master?.getChildByLabel(RoomEngine.OVERLAY) as Container) ?? null;
    }
    ...
    private getOverlayIconSprite(container: Container, label: string): Sprite
    {
        ...
        return container.getChildByLabel(label) as Sprite ?? null;
    }
